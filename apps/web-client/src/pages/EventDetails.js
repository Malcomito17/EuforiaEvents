...
import { Calendar, MapPin, Music2, Mic2, Instagram, Facebook, Twitter, Globe, ExternalLink } from 'lucide-react';
    ...
    return (<div className="min-h-screen pb-20">
      ...
      <main className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <div className="card text-center">
          ...
          {event.eventData?.eventType && (<p className="text-white/60">{event.eventData.eventType}</p>)}
          {event.eventData?.hashtag && (<p className="text-primary-400 mt-2 font-medium">
        ...
        <div className="card space-y-4">
          ...
          {/* Dates */}
          {event.startDate && (<div className="flex items-start gap-3">
          ...
          {event.endDate && (<div className="flex items-start gap-3">
              <Calendar className="w-5 h-5 text-primary-400 mt-0.5 flex-shrink-0"/>
              <div>
          ...
          {/* Venue */}
          {event.venue && (<div className="flex items-start gap-3">
        ...
        {/* Social Media */}
        {(socialMedia.instagram || socialMedia.facebook || socialMedia.twitter || socialMedia.website) && (<div className="card">
            <h2 className="text-lg font-semibold mb-4">Redes Sociales</h2>
            <div className="grid grid-cols-2 gap-3">
              {socialMedia.instagram && (<a href={socialMedia.instagram} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-3 bg-white/5 hover:bg-white/10 rounded-lg transition-colors">
                  <Instagram className="w-5 h-5 text-pink-400"/>
                  <span className="text-sm">Instagram</span>
                  <ExternalLink className="w-3 h-3 ml-auto text-white/40"/>
                </a>)}

              {socialMedia.facebook && (<a href={socialMedia.facebook} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-3 bg-white/5 hover:bg-white/10 rounded-lg transition-colors">
                  <Facebook className="w-5 h-5 text-blue-400"/>
                  <span className="text-sm">Facebook</span>
                  <ExternalLink className="w-3 h-3 ml-auto text-white/40"/>
                </a>)}

              {socialMedia.twitter && (<a href={socialMedia.twitter} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-3 bg-white/5 hover:bg-white/10 rounded-lg transition-colors">
                  <Twitter className="w-5 h-5 text-sky-400"/>
                  <span className="text-sm">Twitter</span>
                  <ExternalLink className="w-3 h-3 ml-auto text-white/40"/>
                </a>)}

              {socialMedia.website && (<a href={socialMedia.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-3 bg-white/5 hover:bg-white/10 rounded-lg transition-colors">
        ...
        <div className="card">
          ...
          <div className="space-y-3">
            {musicadjConfig?.enabled && (<div className="flex items-center gap-3 p-3 bg-white/5 rounded-lg">
                ...
                {musicadjConfig.spotifyAvailable && (<span className="text-xs bg-green-500/20 text-green-300 px-2 py-1 rounded-full">
                    Spotify
                  </span>)}
              </div>)}

            {karaokeyaConfig?.enabled && (<div className="flex items-center gap-3 p-3 bg-white/5 rounded-lg">
        ...
        {musicadjConfig?.enabled && musicadjConfig.welcomeMessage && (<div className="card">
