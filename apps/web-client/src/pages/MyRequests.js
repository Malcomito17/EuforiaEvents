...
import { Music2, Loader2, AlertCircle, Plus, Clock, Star, Zap, CheckCircle2, XCircle } from 'lucide-react';
...
export default function MyRequests() {
    ...
    useEffect(() => {
        ...
        newSocket.on('musicadj:requestUpdated', (updatedRequest) => {
        ...
        newSocket.on('musicadj:newRequest', (newRequest) => {
            if (newRequest.guestId === guest.id) {
                setRequests((prev) => [newRequest, ...prev]);
            }
        });
        // Listen for deleted requests
        newSocket.on('musicadj:requestDeleted', (deletedId) => {
    ...
    return (<div className="min-h-screen pb-safe">
      ...
      <main className="max-w-lg mx-auto px-4 py-6">
        ...
        {!loading && requests.length === 0 && (<div className="text-center py-12">
            ...
            <button onClick={() => navigate(`/e/${slug}/musicadj`)} className="btn-primary inline-flex items-center gap-2">
        ...
        {!loading && requests.length > 0 && (<div className="space-y-6">
            ...
            <div className="space-y-3">
              {requests.map((request) => (<div key={request.id} className="card">
                  <div className="flex items-start gap-4">
                    {/* Album art */}
                    {request.albumArtUrl ? (<img src={request.albumArtUrl} alt={request.title} className="w-16 h-16 rounded-lg flex-shrink-0"/>) : (<div className="w-16 h-16 bg-white/10 rounded-lg flex items-center justify-center flex-shrink-0">
                        <Music2 className="w-8 h-8 text-white/40"/>
                      </div>)}
                  ...
                  {request.status === 'HIGHLIGHTED' && (<p className="mt-3 text-sm text-blue-300 bg-blue-500/10 px-3 py-2 rounded-lg">
                      El DJ destacó tu pedido
                    </p>)}
                  {request.status === 'URGENT' && (<p className="mt-3 text-sm text-orange-300 bg-orange-500/10 px-3 py-2 rounded-lg">
                      ¡Tu pedido está marcado como urgente!
                    </p>)}
                  {request.status === 'PLAYED' && (<p className="mt-3 text-sm text-green-300 bg-green-500/10 px-3 py-2 rounded-lg">
                      Este tema ya fue reproducido
                    </p>)}
                  {request.status === 'DISCARDED' && (<p className="mt-3 text-sm text-gray-400 bg-gray-500/10 px-3 py-2 rounded-lg">
            ...
            {/* Add another button */}
            <button onClick={() => navigate(`/e/${slug}/musicadj`)} className="btn-primary w-full flex items-center justify-center gap-2">
              <Plus className="w-5 h-5"/>
              Pedir otro tema
            </button>
          </div>)}
