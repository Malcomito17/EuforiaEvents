...
export default function DisplayScreen() {
    ...
    useEffect(() => {
        if (data?.config.displayMode !== 'QUEUE' || data.config.displayLayout !== 'HORIZONTAL')
            return;
    ...
    // Loading state
    if (loading) {
        return (<div className="min-h-screen bg-gradient-to-br from-purple-900 via-purple-800 to-pink-800 flex items-center justify-center">
    ...
    // Error state
    if (error || !data) {
        return (<div className="min-h-screen bg-gradient-to-br from-red-900 via-red-800 to-pink-800 flex items-center justify-center">
    ...
    return (<div className={clsx('min-h-screen relative', 'bg-gradient-to-br from-purple-900 via-purple-800 to-pink-800', 'text-white overflow-hidden')}>
      {/* Fullscreen button (discrete) */}
      <button onClick={toggleFullscreen} className="absolute top-4 right-4 z-50 bg-black/30 hover:bg-black/50 text-white p-2 rounded-lg transition" aria-label="Pantalla completa">
        {isFullscreen ? (<Minimize className="w-5 h-5"/>) : (<Maximize className="w-5 h-5"/>)}
      </button>

      {/* MODO BREAK */}
      {config.displayMode === 'BREAK' && (<div className="flex flex-col items-center justify-center min-h-screen px-8">
      ...
      {/* MODO START */}
      {config.displayMode === 'START' && (<div className="flex flex-col items-center justify-center min-h-screen px-8">
      ...
      {/* MODO PROMO */}
      {config.displayMode === 'PROMO' && (<div className="flex items-center justify-center min-h-screen px-8">
          {config.displayPromoImageUrl ? (<div className="relative w-full max-w-6xl">
              <img src={config.displayPromoImageUrl} alt="Promoción" className="w-full h-auto rounded-3xl shadow-2xl"/>
      ...
      {/* MODO QUEUE */}
      {config.displayMode === 'QUEUE' && (<div className="min-h-screen flex flex-col p-8">
          {/* Header */}
          ...
          {/* Layout VERTICAL */}
          {isVertical && (<div className="flex-1 flex flex-col gap-8">
              {/* Cantante actual */}
              {queue.onStage && (<div className="bg-gradient-to-r from-pink-600 to-pink-700 rounded-3xl p-8 shadow-2xl border-4 border-pink-400">
                  <div className="text-center">
                    ...
                    {queue.onStage.artist && (<p className="text-2xl text-pink-200 mt-2">{queue.onStage.artist}</p>)}
                  </div>
                </div>)}

              {/* Próximo */}
              {queue.called && (<div className="bg-gradient-to-r from-purple-600 to-purple-700 rounded-3xl p-6 shadow-xl border-4 border-purple-400">
                  <div className="text-center">
                    ...
                    {queue.called.artist && (<p className="text-lg text-purple-200">{queue.called.artist}</p>)}
                  </div>
                </div>)}
              ...
              <div className="flex-1 bg-white/10 rounded-3xl p-6 backdrop-blur-sm overflow-y-auto">
                ...
                <div className="space-y-3">
                  {queue.upcoming.slice(0, 6).map((request) => (<div key={request.id} className="bg-white/20 rounded-xl p-4 backdrop-blur-sm">
          ...
          {/* Layout HORIZONTAL */}
          {!isVertical && (<div className="flex-1 flex gap-6">
              {/* Izquierda: Cantante actual + Próximo */}
              <div className="w-1/2 flex flex-col gap-6">
                {/* Cantante actual */}
                {queue.onStage && (<div className="flex-1 bg-gradient-to-br from-pink-600 to-pink-700 rounded-3xl p-6 shadow-2xl border-4 border-pink-400 flex flex-col justify-center">
                    <div className="text-center">
                      ...
                      {queue.onStage.artist && (<p className="text-xl text-pink-200 mt-1">{queue.onStage.artist}</p>)}
                    </div>
                  </div>)}

                {/* Próximo */}
                {queue.called && (<div className="flex-1 bg-gradient-to-br from-purple-600 to-purple-700 rounded-3xl p-6 shadow-xl border-4 border-purple-400 flex flex-col justify-center">
                    <div className="text-center">
              ...
              <div className="w-1/2 flex flex-col gap-6">
                ...
                <div className="flex-1 bg-white/10 rounded-3xl p-6 backdrop-blur-sm overflow-hidden">
                  ...
                  <div className="space-y-3 overflow-y-auto max-h-[500px]">
                    {queue.upcoming.slice(0, 8).map((request, index) => (<div key={request.id} className={clsx('bg-white/20 rounded-xl p-4 backdrop-blur-sm transition-all duration-500', index === scrollPosition && 'scale-105 bg-white/30')}>
