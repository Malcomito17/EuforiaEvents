...
import { Mic2, Loader2, AlertCircle, Plus, Clock, Phone, Users, CheckCircle2, UserX, XCircle, Trash2 } from 'lucide-react';
...
function StatusBadge({ status }) {
    ...
    return (<span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${className}`}>
...
export default function KaraokeQueue() {
    ...
    const handleCancelRequest = async (requestId, title) => {
    ...
    useEffect(() => {
        ...
        newSocket.on('karaokeya:request:updated', (updatedRequest) => {
            setRequests((prev) => {
        ...
        newSocket.on('karaokeya:request:new', (newRequest) => {
        ...
        newSocket.on('karaokeya:request:deleted', (deletedId) => {
    ...
    return (<div className="min-h-screen pb-safe">
      <ClientHeader title="Cola del Evento" subtitle={event.name} showTurnNotification={false}/>

      <main className="max-w-lg mx-auto px-4 py-6">
        {/* Banner "ES TU TURNO" */}
        {hasCalledRequest && (<div className="mb-4 p-4 bg-gradient-to-r from-green-500 via-emerald-500 to-green-500 border-2 border-green-400 rounded-xl animate-pulse">
        ...
        {/* Empty state */}
        {!loading && requests.length === 0 && (<div className="text-center py-12">
            ...
            <button onClick={() => navigate(`/e/${slug}/karaokeya`)} className="btn-primary inline-flex items-center gap-2">
        ...
        {/* Requests list */}
        {!loading && requests.length > 0 && (<div className="space-y-6">
            {/* Stats */}
            ...
            <div className="space-y-3">
              {requests.map((request) => {
                ...
                return (<div key={request.id} className={`card relative ${isMyRequest ? 'border-2 border-primary-500 bg-primary-500/10' : ''}`}>
                    {/* Cancel button - only for user's own requests */}
                    {isMyRequest && (<button onClick={() => handleCancelRequest(request.id, request.title)} className="absolute top-3 right-3 p-2 bg-red-500/20 hover:bg-red-500/30 rounded-full transition-colors group" title="Cancelar pedido">
                        <Trash2 className="w-4 h-4 text-red-400 group-hover:text-red-300"/>
                      </button>)}

                    <div className="flex items-start gap-4">
                      {/* Thumbnail */}
                      {thumbnailUrl ? (<img src={thumbnailUrl} alt={request.title} className="w-16 h-16 rounded-lg flex-shrink-0 object-cover"/>) : (<div className="w-16 h-16 bg-white/10 rounded-lg flex items-center justify-center flex-shrink-0">
                          <Mic2 className="w-8 h-8 text-white/40"/>
                        </div>)}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2 mb-1">
                          ...
                          {request.turnNumber && (<span className="bg-primary-500/20 text-primary-300 border border-primary-500/50 px-2.5 py-0.5 rounded-full text-xs font-bold flex-shrink-0">
                              #{request.turnNumber}
                            </span>)}
                        </div>
                        ...
                        {/* Rating + Difficulty */}
                        {request.song && (request.song.ranking || request.song.difficulty) && (<div className="flex items-center gap-2 mb-2">
                        ...
                        {/* Opinion */}
                        {request.song?.opinion && (<p className="text-xs text-white/50 italic mb-2 line-clamp-2">
                            "{request.song.opinion}"
                          </p>)}
                        <div className="flex items-center gap-2 flex-wrap">
                          <StatusBadge status={request.status}/>
                          {(request.status === 'QUEUED' || request.status === 'CALLED') && (<span className="text-xs text-white/60 bg-white/5 px-2 py-0.5 rounded-full">
                        ...
                        {youtubeShareUrl && (<a href={youtubeShareUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-red-400 hover:text-red-300 mt-2">
                            Ver en YouTube →
                          </a>)}
                      </div>
                    </div>
                    ...
                    {request.status === 'QUEUED' && (<p className="mt-3 text-sm text-yellow-300 bg-yellow-500/10 px-3 py-2 rounded-lg">
                        Estás en la posición {request.queuePosition} de la cola
                      </p>)}
                    {request.status === 'CALLED' && (<p className="mt-3 text-sm text-green-300 bg-green-500/10 px-3 py-2 rounded-lg font-semibold animate-pulse">
                        ¡ES TU TURNO! Acercate al escenario
                      </p>)}
                    {request.status === 'ON_STAGE' && (<p className="mt-3 text-sm text-purple-300 bg-purple-500/10 px-3 py-2 rounded-lg">
                        ¡Estás cantando! 🎤
                      </p>)}
                    {request.status === 'COMPLETED' && (<p className="mt-3 text-sm text-green-300 bg-green-500/10 px-3 py-2 rounded-lg">
                        ¡Genial performance! 👏
                      </p>)}
                    {request.status === 'NO_SHOW' && (<p className="mt-3 text-sm text-orange-300 bg-orange-500/10 px-3 py-2 rounded-lg">
                        No viniste cuando te llamaron
                      </p>)}
                    {request.status === 'CANCELLED' && (<p className="mt-3 text-sm text-gray-400 bg-gray-500/10 px-3 py-2 rounded-lg">
            ...
            {/* Add another button */}
            <button onClick={() => navigate(`/e/${slug}/karaokeya`)} className="btn-primary w-full flex items-center justify-center gap-2">
              <Plus className="w-5 h-5"/>
              Pedir otro tema
            </button>
          </div>)}
...
//# sourceMappingURL=KaraokeQueue.js.map