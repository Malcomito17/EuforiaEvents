    ...
    <header className="sticky top-0 bg-gray-900/80 backdrop-blur-lg border-b border-white/10 z-10">
      <div className="max-w-lg mx-auto px-4 py-3">
        ...
        <div className="flex items-center gap-3 mb-1">
          ...
          {showTurnNotification && hasTurn && (
            <button
              onClick={() => navigate(`/e/${event?.slug}/karaokeya/queue`)}
        ...
        {event && (
          <button
            ...
            className="text-xs text-white/50 hover:text-white/70 transition-colors truncate w-full text-left"
          >
            📍 {event.name}
          </button>
        )}
