...
export function useKaraokeQueue(eventId, guestId, enabled = true) {
    ...
    useEffect(() => {
        ...
        const checkInitialStatus = async () => {
            try {
                const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/events/${eventId}/karaokeya/guests/${guestId}/requests`);
                const data = await response.json();
                const hasCalledRequest = data.requests?.some((req) => req.status === 'CALLED');
...
//# sourceMappingURL=useKaraokeQueue.js.map