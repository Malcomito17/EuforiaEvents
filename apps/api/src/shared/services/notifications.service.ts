...
export interface SendNotificationInput {
  phoneNumber: string  // Formato internacional: +54911XXXXXXXX
  message: string
...
export async function sendWhatsAppNotification(
  ...
  try {
    ...
    console.log(
      `[WHATSAPP] Mensaje enviado a ${normalizedPhone}. SID: ${twilioMessage.sid}`
    )
...
export async function sendSMSNotification(
  ...
  try {
    ...
    console.log(
      `[SMS] Mensaje enviado a ${normalizedPhone}. SID: ${twilioMessage.sid}`
    )
...
export async function sendNotification(
  ...
  if (preferWhatsApp && isWhatsAppConfigured()) {
    ...
    console.log(
      `[NOTIFICATIONS] WhatsApp falló, intentando SMS fallback...`
    )
  }
