...
import { eventsApi, venuesApi, clientsApi, usersApi } from '@/lib/api';
...
export function DashboardPage() {
    ...
    return (<div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        {recentEvents.length === 0 ? (<div className="p-8 text-center text-gray-500">
            No hay eventos aún.{' '}
            <Link to="/events/new" className="text-primary-600 hover:underline">
              Creá el primero
            </Link>
          </div>) : (<div className="divide-y divide-gray-100">
            {recentEvents.map((event) => (<Link key={event.id} to={`/events/${event.id}`} className="block p-4 hover:bg-gray-50 transition-colors">
