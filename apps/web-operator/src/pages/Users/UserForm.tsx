...
export default function UserForm() {
  ...
  const loadRolePreset = (r: string) => {
    usersApi.getRolePreset(r).then(res => {
      setPermissions(res.data.permissions || [])
    })
  }

  const updatePermission = (module: string, field: string, value: boolean) => {
    setPermissions(prev => {
                  ...
                  return (
                    <tr key={mod} className="border-t">
                      ...
                      <td className="px-4 py-2 text-center">
                        <input type="checkbox" checked={perm?.canView || false}
                          onChange={e => updatePermission(mod, 'canView', e.target.checked)} />
                      </td>
                      <td className="px-4 py-2 text-center">
                        <input type="checkbox" checked={perm?.canEdit || false}
                          onChange={e => updatePermission(mod, 'canEdit', e.target.checked)} />
                      </td>
                      <td className="px-4 py-2 text-center">
                        <input type="checkbox" checked={perm?.canDelete || false}
                          onChange={e => updatePermission(mod, 'canDelete', e.target.checked)} />
                      </td>
                      <td className="px-4 py-2 text-center">
                        <input type="checkbox" checked={perm?.canExport || false}
                          onChange={e => updatePermission(mod, 'canExport', e.target.checked)} />
                      </td>
                    </tr>
                  )
                })}
